...
class ForgotPassword extends React.Component{
  ...
  onSubmit(e){
    ...
    if(isValid){
    this.props.resetEmail(this.state).then(res =>this.setState({isLoading:false,reqFormStatus:res.data}))
    .catch(err=>this.setState({isLoading:false,errors:err.response.data}));
    }else{
      this.setState({
        isLoading:false
      });
        ...
        <div className="container-fluid bgSignUP">
          <div className="row h-100 justify-content-center align-items-center bgColor">
            <div className=" col-sm-6 col-md-6 col-lg-4 text-center">
              ...
              {/*  success message */}
              {this.state.reqFormStatus && <p className="alert alert-success">{this.state.reqFormStatus}</p>}
                {/*   loading */}
...
export default withRouter(connect(null,{resetEmail})(ForgotPassword));
